...
struct cascadeCorrelations {
  ...
  void init(InitContext const&){
    ...
    // LOGF(info, "histo name %s", hEffXiMin->GetName());
  }
                         ...
                         aod::V0sLinked const&, aod::V0Datas const&, FullTracksExtIU const&)
  {
    ...
    for (auto& [col1, cascades1, col2, cascades2] : pair) {
      ...
      if (col1.globalIndex() == col2.globalIndex()){
        LOGF(info, "Uh oh, same collision! %d, %d", col1.globalIndex(), col2.globalIndex());
        continue;
      }
      ...
      for (auto& [casc1, casc2] : combinations(CombinationsFullIndexPolicy(cascades1, cascades2))) {
        ...
        if (trigger.collisionId() == assoc.collisionId()){
          LOGF(info, "Uh oh, same collision in casc loop! %d, %d", trigger.collisionId(), assoc.collisionId());
          continue; 
        }

        // double deta = trigger.eta() - assoc.eta(); TODO: REMOVE OBSOLETE?
        double dphi = RecoDecay::constrainAngle(trigger.phi() - assoc.phi(), -0.5 * PI);
        ...
        if (trigger.sign() * assoc.sign() < 0) { // opposite-sign
          ...
          // registry.fill(HIST("MixedEvents/hMEXiXiOS"), dphi, deta, trigger.pt(), assoc.pt(), invMassXiTrigg, invMassXiAssoc, col1.posZ(), col1.multFT0M(), weight);
          // registry.fill(HIST("MixedEvents/hMEXiOmOS"), dphi, deta, trigger.pt(), assoc.pt(), invMassXiTrigg, invMassOmAssoc, col1.posZ(), col1.multFT0M(), weight);
          // registry.fill(HIST("MixedEvents/hMEOmXiOS"), dphi, deta, trigger.pt(), assoc.pt(), invMassOmTrigg, invMassXiAssoc, col1.posZ(), col1.multFT0M(), weight);
          // registry.fill(HIST("MixedEvents/hMEOmOmOS"), dphi, deta, trigger.pt(), assoc.pt(), invMassOmTrigg, invMassOmAssoc, col1.posZ(), col1.multFT0M(), weight);
          // Fill the different THnSparses depending on PID logic (important for rapidity & inv mass information)
          if (trigger.isSelected() <= 2) { // trigger Xi
            if(doEfficiencyCorrection) weightTrigg = trigger.sign() < 0 ? 1. / getEfficiency(hEffXiMin, trigger.pt()) : 1. / getEfficiency(hEffXiPlus, trigger.pt());
            if (assoc.isSelected() <= 2) { // assoc Xi
          ...
          /// vvv DEBUG REMOVE vvv
          if (TMath::Abs(dphi) < 0.2){
            // LOGF(info, "SS debug, col1 %d, col2 %d, trigger %d, assoc %d", trigger.collisionId(), assoc.collisionId(), trigger.globalIndex(), assoc.globalIndex());
            LOGF(info, "SS debug track check: pos %d %d, neg %d %d, bach %d %d", trigger.posTrackId(), assoc.posTrackId(), trigger.negTrackId(), assoc.negTrackId(), trigger.bachelorId(), assoc.bachelorId());
            if (trigger.posTrackId() == assoc.posTrackId()){
              auto triggerPosTrack = trigger.posTrack_as<FullTracksExtIU>();
              auto assocPosTrack = assoc.posTrack_as<FullTracksExtIU>();
              LOGF(info, "SS debug pos track collision: tracks %d %d, cascades %d %d", triggerPosTrack.collisionId(), assocPosTrack.collisionId(), trigger.collisionId(), assoc.collisionId());
              LOGF(info, "SS debug pos track collision: nITS trig assoc %d %d", triggerPosTrack.itsNCls(), assocPosTrack.itsNCls());
            }
          }
          /// ^^^ DEBUG REMOVE ^^^
          ...
          // registry.fill(HIST("MixedEvents/hMEXiXiSS"), dphi, deta, trigger.pt(), assoc.pt(), invMassXiTrigg, invMassXiAssoc, col1.posZ(), col1.multFT0M(), weight);
          // registry.fill(HIST("MixedEvents/hMEXiOmSS"), dphi, deta, trigger.pt(), assoc.pt(), invMassXiTrigg, invMassOmAssoc, col1.posZ(), col1.multFT0M(), weight);
          // registry.fill(HIST("MixedEvents/hMEOmXiSS"), dphi, deta, trigger.pt(), assoc.pt(), invMassOmTrigg, invMassXiAssoc, col1.posZ(), col1.multFT0M(), weight);
          // registry.fill(HIST("MixedEvents/hMEOmOmSS"), dphi, deta, trigger.pt(), assoc.pt(), invMassOmTrigg, invMassOmAssoc, col1.posZ(), col1.multFT0M(), weight);
          if (trigger.isSelected() <= 2) { // trigger Xi
            if(doEfficiencyCorrection) weightTrigg = trigger.sign() < 0 ? 1. / getEfficiency(hEffXiMin, trigger.pt()) : 1. / getEfficiency(hEffXiPlus, trigger.pt());
            if (assoc.isSelected() <= 2) { // assoc Xi
              if(doEfficiencyCorrection) weightAssoc = assoc.sign() < 0 ? 1. / getEfficiency(hEffXiMin, assoc.pt()) : 1. / getEfficiency(hEffXiPlus, assoc.pt());
